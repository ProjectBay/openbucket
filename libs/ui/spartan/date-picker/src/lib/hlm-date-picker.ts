...
import { HlmCalendar } from '@openbucket/spartan-ui/calendar';
import { HlmIconImports } from '@openbucket/spartan-ui/icon';
import { HlmPopoverImports } from '@openbucket/spartan-ui/popover';
import { hlm } from '@openbucket/spartan-ui/utils';
import type { ClassValue } from 'clsx';
import { injectHlmDatePickerService } from './hlm-date-picker.token';
...
export class HlmDatePicker<T> implements ControlValueAccessor {
  private readonly _configService = injectHlmDatePickerService<T>();
  ...
  public readonly autoCloseOnSelect = input<boolean | undefined, BooleanInput>(
    undefined,
    {
      transform: booleanAttribute,
    },
  ...
  public readonly formatDate = input<((date: T) => string) | undefined>(
    undefined,
  );

  /** Defines how the date should be transformed before saving to model/form. */
  public readonly transformDate = input<((date: T) => T) | undefined>(
    undefined,
  );

  /** Computed value that uses input or falls back to service config */
  protected readonly _effectiveAutoCloseOnSelect = computed(
    () =>
      this.autoCloseOnSelect() ??
      this._configService.config().autoCloseOnSelect,
  );

  /** Computed value that uses input or falls back to service config */
  protected readonly _effectiveFormatDate = computed(
    () => this.formatDate() ?? this._configService.config().formatDate,
  );

  /** Computed value that uses input or falls back to service config */
  protected readonly _effectiveTransformDate = computed(
    () => this.transformDate() ?? this._configService.config().transformDate,
  );
  ...
  protected readonly _formattedDate = computed(() => {
    ...
    return date ? this._effectiveFormatDate()(date) : undefined;
  });
  ...
  protected _handleChange(value: T) {
    ...
    const transformedDate =
      value !== undefined ? this._effectiveTransformDate()(value) : value;

    this._mutableDate.set(transformedDate);
    this._onChange?.(transformedDate);
    this.dateChange.emit(transformedDate);

    if (this._effectiveAutoCloseOnSelect()) {
  ...
  public writeValue(value: T | null): void {
    this._mutableDate.set(
      value ? this._effectiveTransformDate()(value) : undefined,
    );
  }
