...
import { HlmSheetImports } from '@openbucket/spartan-ui/sheet';
import { classes, hlm } from '@openbucket/spartan-ui/utils';
...
export class HlmSidebar {
  ...
  protected readonly _sidebarGapComputedClass = computed(() =>
    hlm(
      ...
      // Compact variant: no gap needed as sidebar is always visible
      this.variant() === 'compact' ? 'w-[var(--sidebar-width)]' : '',
    ),
  );
