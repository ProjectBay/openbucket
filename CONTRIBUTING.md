...
setup, the (slightly unusual) Node requirements, and the workflow for changes.
...
- **Node.js 22** (`>= 22.12`) — a single version runs the whole stack: the
  ...
  what CI and the Docker image use and is pinned in [`.nvmrc`](./.nvmrc):
...
Releases are automated: pushing a `nestjs-v<version>` tag triggers
[`.github/workflows/release-nestjs.yml`](./.github/workflows/release-nestjs.yml).

## Code style

- **Prettier** + **ESLint** are enforced; run `nx run-many -t lint` and
  `npx prettier --write .` before pushing. An [`.editorconfig`](./.editorconfig)
  keeps editors consistent (LF, UTF-8, 2-space indent).
...
4. Open a PR against `main`. **CI must be green** — it's the functional gate
   (the release workflow deliberately skips the unit suite because a couple of
   concurrency specs are racy, so don't rely on that).
...
and the [whitepaper](./docs/WHITEPAPER.md) for the architecture.
